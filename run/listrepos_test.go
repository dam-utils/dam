...
2||test1|test.com|
`
	db.NewRepo(&storage.Repo{2, false, "test1", "test.com", "", ""})
	log.SetOutput(&buf)
	run.ListRepos()
	log.SetOutput(os.Stdout)
	printFailInfo(t, stdout, buf.String())
...
`
	db.NewRepo(&storage.Repo{2, false, "test1", "test.com", "", ""})
	db.NewRepo(&storage.Repo{5, false,"test2","test2.com","user","user"})
	db.NewRepo(&storage.Repo{0, true,"test2","test2.com","user","user"})
	log.SetOutput(&buf)
	run.ListRepos()
	log.SetOutput(os.Stdout)
	printFailInfo(t, stdout, buf.String())
