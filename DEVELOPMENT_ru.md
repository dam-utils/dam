...
## Changelog
0.0.x - ветка планируется доработки:
- багфиксы dam search (se) и работы с репозиториями
- возможно переименование пакетов и пересмотр архитектуры

1.x.x - в ветка планируется развиваться паралельно с 0.0.x
...
- dam import - развертывание нескольких приложений с версиями, обьединенных в одну группу (продукт)
- dam export - сохранение списка или самих приложений в файл
- dam purge - удаление из локального кэша докера неиспользуемых образов (освобожедение места от старых версий приложений) 
Дополнительно:
...
(возможно стоит делать запроз к server docker, брать версию и сравнивать со своим диапазоном)
- проверить работу с файлами в files_db (возможно неправильное закрытия файла)
- запретить паралельную работу с базой, добавить блокировки (по сути на одной системе, работа команды dam должна быть атомарна)
...
Переделать/убрать пакет d_log на более удобный.
- dam search возможно, стоит добавить рутины на несколько http запросов (ускорит медленный поиск в оффициальном репозитории)
...
При первом запуске dim видит, что нет в базе записе о репозиториях, создает новую
```
...
- Флаг --server:
  Для оффициального registry - это имя сервера, для локального не только имя, но и префикс приложения до знака "/"
...
- Id:
  ...
  Id с номером 0 - оффициальный репозиторий.
